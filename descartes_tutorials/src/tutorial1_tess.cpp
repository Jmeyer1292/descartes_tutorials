...
descartes_core::TrajectoryPtPtr makeTolerancedCartesianPoint(const Eigen::Affine3d& pose, double dt)
{
  using namespace descartes_core;
  using namespace descartes_trajectory;
  return TrajectoryPtPtr( new AxialSymmetricPt(pose, M_PI / 36.0, AxialSymmetricPt::Z_AXIS, TimingConstraint(dt)) );
}
...
std::vector<descartes_core::TrajectoryPtPtr> makePath()
{
  ...
  const static int num_steps = 200;
  const static double time_between_points = 0.05;
